...
test('하네스가 주입한 시스템 메시지는 채점하지 않는다 (실사례)', () => {
  // 실제로 8점이 기록됐던 백그라운드 작업 완료 알림
  assert.strictEqual(
    shouldSkip(
      '[SYSTEM NOTIFICATION - NOT USER INPUT]\nThis is an automated background-task event.\n' +
        '<task-notification>\n<task-id>bowp68mdw</task-id>\n<status>completed</status>\n</task-notification>'
    ),
    true
  );

  // 슬래시 명령어 래퍼와 로컬 명령 출력
  assert.strictEqual(
    shouldSkip('<command-name>/model</command-name>\n<command-message>model</command-message>'),
    true
  );
  assert.strictEqual(
    shouldSkip(
  ...
  assert.strictEqual(shouldSkip('<system-reminder>배경 컨텍스트입니다</system-reminder>'), true);

  // 사람이 쓴 프롬프트는 영향받지 않는다
  assert.strictEqual(shouldSkip('신규 요금제 안내문을 표 3행으로 작성해줘'), false);
});
